...
This code can be used to extract information from data.gov using the CKAN API
 it currently writes out the following information to a csv file from all datasets 
 nested within collections (collection_package_id:*) indexed in the Federal Data Catalog:
    'Dataset Name'
    'Data.gov Link' 
    'Agency Data Link' 
    'Metadata Created' 
...
def fetch_datagov_inventory(output_file="datagov_collections_inventory.csv", resume=True):
    ...
    # Get total count for verification using the specific collection filter
    try:
        # Changed query to filter by collection_package_id:*
        params = {'fq': 'collection_package_id:*', 'rows': 0}
        total_found = requests.get(base_url, params=params).json()['result']['count']
        print(f"Total collection datasets reported by Data.gov: {total_found}")
...
if __name__ == "__main__":
    ...
    fetch_datagov_inventory(output_file="datagov_collections_inventory.csv", resume=True)